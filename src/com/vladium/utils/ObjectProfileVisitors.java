...
public abstract class ObjectProfileVisitors
{
   ...
   * @param format
   *          percentage formatter to use [null is equivalent to NumberFormat.getPercentInstance (), with a single
   *          fraction digit]
   * @param shortClassNames
   *          'true' causes all class names to be dumped in compact [no package prefix] form
   */
  public static ObjectProfileNode.INodeVisitor newDefaultNodePrinter(final PrintWriter out, final String indent,
      final DecimalFormat format, final boolean shortClassNames)
  {
    return new DefaultNodePrinter(out, indent, format, shortClassNames);
  }

  /**
   * Factory method for creating the XML output visitor. To create a valid XML document, start the traversal on the
   * profile root node. It is up to the caller to buffer 'out'.
   * 
   * @param out
   ...
   *          'true' causes all class names to be dumped in compact [no package prefix] form
   */
  public static ObjectProfileNode.INodeVisitor newXMLNodePrinter(final OutputStream out, final String indent,
      final DecimalFormat format, final boolean shortClassNames)
  {
  ...
  /**
   * This visitor prints out a node in plain text format. The output is indented according to the length of the node's
   * path within its profile tree.
   */
  ...
  /*
   * This visitor can dump a profile tree in an XML file, which can be handy for examination of very large object
   * graphs.
   */
