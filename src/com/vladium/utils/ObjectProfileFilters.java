...
public abstract class ObjectProfileFilters
{
  ...
  /**
   * Factory method for creating a visitor that only accepts profile nodes with sizes larger than a given threshold
   * value.
   * 
   * @param threshold
   *          node size in bytes
  ...
  /**
   * Factory method for creating a visitor that accepts a profile node only if it is at least the k-th largest child of
   * its parent for a given value of k. E.g., newRankFilter(1) will prune the profile tree so that only the largest
  ...
  /**
   * Factory method for creating a visitor that accepts a profile node only if its size is larger than a given threshold
   * relative to the size of the root node (i.e., size of the entire profile tree).
   * 
   * @param threshold
  ...
  /**
   * Factory method for creating a visitor that accepts a profile node only if its size is larger than a given threshold
   * relative to the size of its parent node. This is useful for pruning the profile tree to show the largest
   * contributors at every tree level.
   * 
   * @param threshold
   *          size fraction threshold
   */
  ...
  private static final class SizeFilter implements IObjectProfileNode.INodeFilter
  {
    public boolean accept(final IObjectProfileNode node)
    {
      ...
      if ("com.scholarone.persistence.valueobject.SQLPersistence#cacheManager".equalsIgnoreCase(nodeName)||
          "com.scholarone.persistence.valueobject.ValueObjectList#owner".equalsIgnoreCase(nodeName))
      {
        return false;
      }
