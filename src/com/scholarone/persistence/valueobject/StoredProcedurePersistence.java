...
public class StoredProcedurePersistence extends SQLPersistence
{
  ...
  {      
    ...
    catch (SQLException se)
    {
      ...
      throw new PersistenceException(se.toString(), se.getSQLState());
    }
    finally
    {
      ...
      while (hasResultSet && moreResults)
      {
        ...
        if ((i > 0) && (list != null) && (parentList != null))
        {
          ...
          for (ValueObject newOb : list) // For each child...
          {
            ...
            if ( parentId == null )
            {
              log.error("Null parentId. newOb = " 
                  + (newOb == null ? "null" : newOb.toString()) 
                  + ", attrChild = " + (attrChild == null ? "null" : attrChild.toString()));
            }
            else
            {
  ...
  public Integer copy(ValueObjectConfiguration config, Vector keys) throws PersistenceException
  {
    return (copy(null, config, keys));
  }
  
  public Integer copy(Connection con, ValueObjectConfiguration config, Vector keys) throws PersistenceException
  {
  ...
  public void save(Connection con, ValueObject object, String methodName) throws PersistenceException
  {
  ...
  public void batchUpdate(ValueObjectList list) throws PersistenceException
  {
   ...
   * @throws PersistenceException
   */
  public void batchUpdate(ValueObjectList list, String methodName) throws PersistenceException
  {
    ...
    catch (SQLException se)
    {
      ...
      throw new PersistenceException(se.getMessage(), se.getSQLState());
    }
    finally
    {
      // close connections
