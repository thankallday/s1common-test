...
import com.scholarone.persistence.helpers.StoredProcedure;
...
/**
 * A fully SAX2-compliant reader for XML configuration files. Makes use of inner classes to process the elements in the
 * document.
 */
public class ValueObjectConfigurationReader
{
  ...
  {
    log = Logger.getLogger(ValueObjectConfigurationReader.class);
  }

  public ValueObjectConfiguration readConfiguration(Object valueOb) throws XMLConfigurationException
  {
    ...
    {
      log.warn("Caught XML Parse error: ", spe);
      spe.printStackTrace();

      String err = "File " + XMLUtils.getConfigFileName(valueOb) + " Line " + spe.getLineNumber() + ", Column "
          + spe.getColumnNumber();

      log.debug("Caught XML parse error: " + err + ", " + spe.getMessage());

      throw new XMLConfigurationException(err);
    }
    catch (SAXException se)
    {
      log.debug("Caught XML parse error in: " + XMLUtils.getConfigFileName(valueOb) + ", " + se.getMessage());

      throw new XMLConfigurationException(se.toString());
    }
    catch (IOException ioe)
    {
      log.warn("Caught XML parse error in: " + XMLUtils.getConfigFileName(valueOb), ioe);
      log.debug("Caught XML parse error in: " + XMLUtils.getConfigFileName(valueOb) + ", " + ioe.toString());

      throw new XMLConfigurationException(ioe.toString());
    }
  ...
  static class FileStorageObjectHandler extends AbstractHandler
  {
    ...
    public void init(String namespace, String localName, String qName, Attributes attrs) throws SAXParseException
    {
      log.debug("Entering FileStorageObjectHandler.init");
  ...
  static class FieldMappingHandler extends AbstractHandler
  {
    ...
    public void init(String namespace, String localName, String qName, Attributes attrs) throws SAXParseException
    {
      log.debug("Entering FieldMappingHandler.init");
