...
public class CacheObjectFlushLRUThread extends Thread
{
      ...
      Boolean stopOnError) throws RuntimeException
  {
    ...
    log = Logger.getLogger(CacheObjectFlushLRUThread.class);
  ...
  public void stopLRUFlushing()
  {
    ...
    log.debug("<threadStop>" + "<serverName>" + this.serverName + "</serverName>" + "<endTime>" + endTime
        + "</endTime>" + "<message>LRU flushing thread stopped</message>" + "</threadStop>");
  }
  ...
  public void run()
  {
    if (!threadStatus.equals(RUNNING))
    {
      ...
      log.debug("<threadStart>" + "<serverName>" + this.serverName + "</serverName>" + "<startTime>" + startTime
          + "</startTime>" + "<message>LRU flushing thread now running</message>" + "<parameters>" + "<cacheMaxSize>"
          + this.cacheMaxSize + "</cacheMaxSize>" + "<cacheFrequency>" + this.cacheLruRunFrequency
          + "</cacheFrequency>" + "<stopOnError>" + this.stopOnError + "</stopOnError>" + "</parameters>"
          + "</threadStart>");
      this.cacheMBean.updateThreadStatus(this.threadStatus);
    }
    while (threadStatus.equals(RUNNING))
    {
      ...
      catch (Throwable t)
      {
        ...
        log.error(this.serverName + ":Cache LRU", t);
        this.cacheMBean.updateThreadStatus(this.threadStatus);
      }
    }
  }
