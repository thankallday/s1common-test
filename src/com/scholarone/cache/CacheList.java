...
/**
 * A CacheList is like an ArrayList, but only stores indirect links to cacheable objects. If one attempts to add (or use
 * set) as CacheableObject to the list, and the object's isCacheable() method returns true, then the primary-key for the
 * object is stored in the list, and the object is kept in the cache. This allows the object to be flushed from the
 * cache if that is necessary, and the object can be recreated when needed.
 * <P>
 * If a key is stored instead of an object, then the get() method looks the object up in the Cache and returns it,
 * instead of the key.
 * 
 * @author kenk
 * 
 * RPR 7/22/2004: Please do not add a logger to this class, because it is serialized to the applet and the applet can
 * not run a logger.
 * 
 * @uml.dependency supplier="com.scholarone.cache.CacheableObjectHolder.getKey"
 * @uml.dependency supplier="com.scholarone.cache.CacheableObjectHolder"
 */
