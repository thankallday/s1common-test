...
/**
 * A list that keeps track of CachePointers, by when they are accessed. Whenever a CachePointer is accessed, the
 * add(object) method should be called. When it comes time to expire objects, the method clearBefore(time) should be
 * called, and that will return objects that were last accessed before the indicated time. Those objects can then be
 * dropped from the cache. Any object returned by clearBefore() should no longer be in the time-list.
 * <P>
 * It is important that every time an object's access-time is changed that it is re-added to this list, or else the
 * object may never be returned by the clearBefore() method.
 * </P>
 * 
 */
class TimeList 
